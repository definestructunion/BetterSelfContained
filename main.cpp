...
#define MANAGED_ASSEMBLY "Hedgemen.dll"
...
#define CORECLR_FILE_NAME "runtime\\coreclr.dll"
#elif LINUX
#include <dirent.h>
#include <dlfcn.h>
#include <limits.h>
...
int main(int argc, const char **argv)
{
    ...
    if (coreClr == NULL)
    {
        //printf("ERROR: Failed to load CoreCLR %s\n", coreClrPath.c_str());
        //return -1;
        return GetLastError();
    }
    else
    ...
    std::string runtimeFolder = std::string(runtimePath).append("\\runtime");

    BuildTpaList(runtimeFolder.c_str(), ".dll", tpaList);
    ...
    /*hr = createManagedDelegate(
            hostHandle,
            domainId,
            "Hedgemen, Version=1.0.0.0",
            "Hedgemen.ManagedWorker",
            "Main",
            (void**)&managedDelegate);*/
...
void BuildTpaList(const std::string& directory, const char* extension, std::string& tpaList)
{
